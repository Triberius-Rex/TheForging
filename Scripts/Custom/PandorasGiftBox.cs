...
namespace Server.Customs
{
	public class PandorasGiftBox : RandomGiftBox
	{
		...
		private HashSet<Mobile> m_MobilesGivenTo = new HashSet<Mobile>();
		private HashSet<string> m_AccountsGivenTo = new HashSet<string>();
		...
		private void Dupe(Container pack, Item copy)
		{
			bool done = false;
			...
			if (c != null)
			{
				try
				{
					...
					if (o != null && o is Item)
					{
						...
						if(copy is Container)
						{
							DupeContainer((Container)copy, (Container)newItem);
						}
					}
					done = true;
				}
		...
		private void DupeContainer(Container from, Container to)
		{
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			writer.WriteMobileList(new ArrayList(m_MobilesGivenTo.ToArray<Mobile>()));
			string[] accounts = m_AccountsGivenTo.ToArray<string>();
