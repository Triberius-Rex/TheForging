    ...
    public class ReferralTicket : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from is PlayerMobile pm && BlessedFor == pm)
            {
                if (!pm.Young)
                {
                    from.SendMessage(53, "Target the friend who sent you to receive 500 Soveriegns!");
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is PlayerMobile pm && !pm.Young)
                {
                    if (pm != from)
                    {
                        ...
                        from.AddToBackpack(new HeritageSovereign(500));

                        from.SendMessage(53, "You received a referral soveriegn reward!");
